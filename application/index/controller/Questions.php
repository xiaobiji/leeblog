...
//                ->JOIN('member n','n.id=q.rid','left')
//                ->where('q.qid',$id)
//                ->field('q.*,m.username questioner,n.username answerer')
//                ->find();
            if(request()->isPost()){
                $data=input('post.');
                $data['topic_id']=  $id;
                ...
                if(empty($id) || empty($data['content'])){
                    return $this->json(301,[],'参数异常');
                }
                //后端数据验证
...
//                }
                ...
                Lang::set('Content addition success','评论添加成功','zh-cn');
                Lang::set('Content addition failure','评论添加失败','zh-cn');
                if(!$result){
                    return $this->json(302,[],lang('Content addition failure'));
                }
                ...
                $result = model('Article')->allowField(true)->save($q_data,['id' => $id]);
                if(!$result){
                    return $this->json(303,[],'评论次数更新失败');
                }
                return $this->json(201,[],'评论成功');

            }
        return $this->json(304,[],'违法提交');
    }
...
//            Lang::set('Content addition failure','回答失败','zh-cn');
    ...
    public function like(){
        if(request()->isPost()){
            ...
            $like_id = db('like')
                ->where([$seach_id =>$id,'uid'=>$this->user_id])
                ->field('id')
                ->find();
            if($like_id['id']){
